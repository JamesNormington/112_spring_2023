...
<details>
  <summary>Solution</summary>
```{r, collapse=TRUE}
#View() opens up a new tab with a spread sheet preview of the data to visually explore the data. It is commented out in the Rmarkdown file because this is an interactive feature
#View(fifa)  
...
Data are also only useful if we know what they measure!  The `fifa` data table is *tidy* -- it doesn't have any helpful notes.  
...
2. [R Markdown Cheatsheet](https://github.com/rstudio/cheatsheets/raw/main/rmarkdown-2.0.pdf) or in RStudio: Go to Help > Cheat Sheets > R Markdown Cheat Sheet
3. [R  Markdown Reference Guide](https://www.rstudio.com/wp-content/uploads/2015/03/rmarkdown-reference.pdf) or in RStudio: Go to Help > Cheat Sheets > R Markdown Reference Guide
...
b. Make sure you can compile (Knit) the Markdown into a webpage (html file).  
c. Add a new line between `title` and `output` that reads: `author: Your Name`.
d. Create a brief essay about your favorite food. Make sure to include:    
...
There's a data set named `comic_characters` in the `fivethirtyeightdata` package. Check out the codebook (hint: use ?) to understand what these data measure. Then add a second section to your R Markdown file,
and then use code chunks and R commands to perform/answer the following tasks/questions:    
