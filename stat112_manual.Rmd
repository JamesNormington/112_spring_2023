...
```{r eval=FALSE,echo=TRUE}
census_pop_est_2018 <- read_csv("https://jamesnormington.github.io/112_fall_2022/data/us_census_2018_state_pop_est.csv") %>%
  separate(state, into = c("dot", "state"), extra = "merge") %>%
  select(-dot) %>%
  mutate(state = str_to_lower(state))
```
...
Below, we create a basic map and add points of interest (the points are a layer on the map). The data are in `favorite_msp`, created below.
...
```{r fig.alt="Interactive map of James' favorite places in the Twin Cities."}
...
```{r fig.alt="Interactive map of James' favorite places in the Twin Cities, connected with a line."}
leaflet(data = favorite_msp) %>%
...
| Function/Operator |                 Action                 |                       Example |
|-------------------------|:---------------------------:|------------------:|
| `leaflet(data)`        |  Create base dynamic plot |   `leaflet(data = favorite_msp) ` |
...
To examine seasonality in birth rates, look at the daily number of births by
...
To examine patterns within the week, make a box plot showing the daily number of births by day of the week. Interpret your results.
...
Holidays <- read_csv("https://jamesnormington.github.io/112_spring_2023/data/US-Holidays.csv") %>%
  mutate(date = as.POSIXct(lubridate::dmy(date)))
```

```{exercise, name="Holidays", label="holidays"}
Now we will  update the plot from Exercise \@ref(exr:two-year) to include the holidays.   
...
You can download a template .Rmd of this activity [here](template_rmd/08-Reshaping_Data_Assign.Rmd). Put the file in the existing folder `Assignment_04` in your `COMP_STAT_112` folder.
...
## Assignment 5 (Part 2): due Wednesday, Feb 22 @ 11:59pm {-}
...
1. Select two columns: the guest name and the year of appearance.
2. Create a new column called `one` with all entries exactly equal to 1.
3. Pivot wider so that each year is its own column. Use the following extra arguments to make sure each entry is only counted once, and no appearances in a year is counted as 0: `values_fn = list(one = length)` and `values_fill = 0`
4. Create a new column called `totalApps`, with the values equal to `rowSums(across(where(is.numeric)))`
5. Sort by total appearances, so that the most appearances is at the top.
6. Use `slice()` to get the top 10 rows.
...
Lesotho <- read_csv("https://jamesnormington.github.io/112_spring_2023/data/Lesotho.csv")
```
...
## Assignment 6 Part 1: due Wednesday, March 1st @ 11:59pm {-}
...
data_site <-
  "https://jamesnormington.github.io/112_spring_2023/data/2014-Q4-Trips-History-Data-Small.rds"
Trips <- readRDS(gzcon(url(data_site)))
Stations <- read_csv("https://jamesnormington.github.io/112_spring_2023/data/DC-Stations.csv")
```
...
## Assignment 6 Part 2: due Wednesday, March 1st @ 11:59pm {-}
